...
from src.utils.net import PreNet
from src.utils.attention import LocationSensitiveAttention
...
class Decoder(nn.Module):
    ...
    def init_frame(self, memory: torch.Tensor):
        decoder_input = Variable(memory.data.new(memory.size(0), self.n_mel_channels).zero_())

        return decoder_input
    
    def init_states(self, memory: torch.Tensor, mask: Optional[torch.Tensor] = None):
    ...
    def decode(self, decoder_input: torch.Tensor):
        """ 
        ...
        self.attention_hidden = F.dropout(self.attention_hidden, p=self.dropout_rate, training=self.training)
        ...
        self.decoder_hidden = F.dropout(self.decoder_hidden, p=self.dropout_rate, training=self.training)
    ...
    def forward(self, memory: torch.Tensor, decoder_inputs: torch.Tensor, mask: torch.Tensor):
        """ 
            memory: Tensor
    ...
    def inference(self, memory: torch.Tensor, max_decoder_steps: int, gate_threshold: float):
        decoder_input = self.init_frame(memory) # (batch_size, n_mel_channels)
