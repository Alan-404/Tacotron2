...
class LocationSensitiveAttention(nn.Module):
    def __init__(self, attention_rnn_dim: int, embedding_dim: int, attention_dim: int, n_filters: int, kernel_size: int) -> None:
        super().__init__()
        self.query_layer = nn.Linear(in_features=attention_rnn_dim, out_features=attention_dim)
        self.memory_layer = nn.Linear(in_features=embedding_dim, out_features=attention_dim)
    ...
    def get_alignment_energies(self, attention_hidden: torch.Tensor, memory: torch.Tensor, attention_weights_cat: torch.Tensor):
        """ 
    ...
    def forward(self, attention_hidden: torch.Tensor, memory: torch.Tensor, attention_weights_cat: torch.Tensor, mask: Optional[torch.Tensor] = None):
        """ 
            attention_hidden: Tensor 
            - (batch_size, attention_rnn_dim)
            memory: Tensor
        ...
        if mask is not None:
            alignments = alignments.masked_fill_(mask, float('-inf')) # (batch_size, n_ctx)

        attention_weights = F.softmax(alignments, dim=-1) # (batch_size, n_ctx)

        attention_output = torch.matmul(attention_weights.unsqueeze(1), memory) # (batch_size, 1, embedding_dim)

        return attention_output.squeeze(1), attention_weights # (batch_size, embedding_dim) (batch_size, n_ctx)
